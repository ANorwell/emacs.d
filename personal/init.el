...
(defun open-notes-dir ()
  ...
  (ztree-dir "~/notes/")
  (revert-buffer))
(global-set-key (kbd "s-n") 'open-notes-dir)
...
(defun toggle-xshould-on ()
  (interactive)
  (replace-regexp "\\(\\s-+\\)should " "\\1xshould "))

(defun toggle-xshould-off ()
  (interactive)
  (replace-regexp "\\(\\s-+\\)xshould " "\\1should "))

(defun toggle-xshould ()
  (interactive)
  (save-excursion
    (goto-char 0)
...
;; disable undo-tree mode due to its giant history files.....
